...
document.addEventListener("DOMContentLoaded", () => {
  // Cek apakah ada parameter ?scroll= di URL
  const params = new URLSearchParams(window.location.search);
  const section = params.get("scroll"); // misal 'about'

  if (section) {
    // Scroll ke atas dulu (biar hero kelihatan)
    window.scrollTo(0, 0);
    ...
    // Setelah jeda (misal 1.5 detik), baru scroll smooth
    setTimeout(() => {
      if (target) {
        const targetY = target.getBoundingClientRect().top + window.scrollY - navbarHeight;
        window.scrollTo({ top: targetY, behavior: "smooth" });
      }
    }, 800); // jeda 1.5 detik agar Hero sempat tampil
  }
});
