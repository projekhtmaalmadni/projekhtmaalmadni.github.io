  ...
  "links": [
    ...
    "work": {
      ...
      "projects": [
        ...
        {"title": "Pegasus Universe", "category": "HR Management App", "image": "assets/pegasus_universe.png", "link": "#", "color_class": "bg-blue-600/60"},
        {"title": "Belanja Smart", "category": "Groceries App", "image": "assets/belanja_smart.png", "link": "#", "color_class": "bg-green-600/60"},
...
function renderNavbar(navData) {
  ...
  const ctaMarkup = `
    <a href="${navData.cta.href}" 
       class="ml-4 bg-white text-black px-6 py-2 rounded-full hover:bg-blue-400 hover:text-white transition-colors">
       ${navData.cta.text}
    </a>
  `;
        ...
        ctaContainer.innerHTML = `
            <a href="${heroData.cta.href}">
                <button class="bg-white text-dark-bg px-6 py-2.5 text-sm sm:text-base rounded-full font-medium 
                    hover:bg-blue-400 hover:text-white transition-colors shadow-sm">
                    ${heroData.cta.text}
                </button>
            </a>
              <a href="#work" 
...
function renderServices(serviceData) {
    ...
    container.innerHTML = serviceData.items.map((item, index) => `
        <div class="service-card rounded-xl p-8 text-left bg-[#0d1e22] border border-white/10 hover:border-teal-400 transition-colors " style="--delay: ${index * 0.15}s;">
            <div class="mb-8">
                <img src="${item.icon}" alt="${item.alt}" class="w-[60px] h-[60px]">
            </div>
...
function renderWork(workData) {
    ...
    container.innerHTML = workData.projects.filter(p => !p.isHidden).map((project) => `
        <a href="${project.link}" class="project-card group scroll-reveal relative block">
            <div class="card-gradient rounded-2xl overflow-hidden relative">
                <div class="relative overflow-hidden">
                    <img src="${project.image}" alt="${project.title} image" class="w-full h-auto object-cover transition-transform duration-500 group-hover:scale-105" />
                    <span class="absolute top-3 left-3 ${project.color_class} text-white font-poppins text-xs px-3 py-1 rounded-2xl">${project.category}</span>
                </div>
            </div>
            <div class="p-8">
      ...
      <a href="${project.link}" class="project-card group scroll-reveal relative block">
        <div class="card-gradient rounded-2xl overflow-hidden relative">
          <div class="relative overflow-hidden">
            ...
            <!-- Overlay -->
            <div class="overlay absolute inset-0 ${project.color_class} hidden md:flex 
              ...
              <span class="flex items-center gap-2 text-white font-poppins font-medium text-lg 
                ...
                View More
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 transition-transform duration-500 group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="absolute bottom-3 right-3 md:hidden scroll-reveal">
              <span class="flex items-center gap-1 ${project.color_class} text-white font-poppins text-sm px-3 py-1 rounded-2xl">
                View More
...
function initNavbar() {
  ...
  // Smooth Scroll dengan offset agar tidak ketutupan navbar
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
  anchor.addEventListener("click", function (e) {
    ...
    if (target) {
      ...
      // Tentukan offset berdasarkan tinggi navbar (responsif)
