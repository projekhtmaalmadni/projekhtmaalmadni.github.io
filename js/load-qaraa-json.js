// load-qaraa-json.js
// Loader untuk Qaraa sections dari JSON data
document.addEventListener("DOMContentLoaded", async () => {
  try {
    console.log('🔄 Loading Qaraa JSON data...');
    
    // Load JSON data
    const response = await fetch('js/qaraa-data.json');
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    const data = await response.json();
    
    console.log('✅ Qaraa JSON data loaded successfully', data);
    
    // Render semua sections
    renderQaraaNavbar(data.qaraa_sections.navbar);
    renderQaraaHome(data.qaraa_sections.home);
    renderQaraaAbout(data.qaraa_sections.about);
    
    // Pastikan technical section ada sebelum render
    if (data.qaraa_sections.technical) {
      console.log('🔄 Rendering technical section...');
      renderQaraaTechnical(data.qaraa_sections.technical);
    } else {
      console.error('❌ Technical section not found in JSON data');
      showTechnicalError();
    }
    
    renderQaraaCta(data.qaraa_sections.cta);
    renderQaraaFooter(data.qaraa_sections.footer);
    renderQaraaButtons(data.qaraa_sections.buttons);
    
    // Initialize scripts after rendering
    initQaraaScripts();
    
  } catch (error) {
    console.error('❌ Error loading Qaraa JSON data:', error);
    showErrorOverlay('Failed to load Qaraa sections data: ' + error.message);
  }
});
...
function renderQaraaTechnical(technicalData) {
  const technicalContainer = document.getElementById('technical');
  if (!technicalContainer) {
    console.error('❌ Technical container not found');
    return;
  }

  console.log('🔄 Rendering technical section with data:', technicalData);
  ...
  try {
    ...
    console.log('✅ Technical section rendered successfully');
    
  } catch (error) {
    console.error('❌ Error rendering technical section:', error);
    showTechnicalError();
  }
}

// Fungsi render lainnya tetap sama...
function renderQaraaNavbar(navData) {
  const navbarContainer = document.getElementById('navbar');
  if (!navbarContainer) return;

  navbarContainer.innerHTML = `
      <nav id="${navData.id}" 
          class="${navData.classes}">
      
      <div class="relative flex items-center justify-between">
        <!-- Logo -->
        <div class="flex items-center space-x-2">
          <a href="${navData.logo.href}">
            <img src="${navData.logo.src}" alt="${navData.logo.alt}" class="w-[120px] h-auto ml-3" />
          </a>
        </div>

        <!-- Desktop Menu -->
        <div class="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 
                    items-center space-x-10 text-sm font-medium text-white">
          ...
          navData.cta
            ? `<div class="hidden md:flex items-center">
                 <a href="${navData.cta.href}" 
                    class="bg-white text-black px-6 py-[8px] rounded-full hover:bg-purple-300 hover:text-white transition-colors">
                    ${navData.cta.text}
                 </a>
               </div>`
            : ''
...
function renderQaraaHome(homeData) {
...
function renderQaraaAbout(aboutData) {
...
function renderQaraaCta(ctaData) {
...
function renderQaraaFooter(footerData) {
          ...
          <div class="space-y-5">
            ${footerData.content.contact.items.map(item => `
              <div class="flex items-start space-x-3">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-purple-400 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ${item.type === 'location' ? `
...
function renderQaraaButtons(buttonsData) {
    ...
    technicalContainer.innerHTML = `
      <section class="py-20 bg-dark-bg">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div class="bg-red-500/10 border border-red-500 rounded-xl p-8">
            <h2 class="text-2xl font-semibold text-red-400 mb-4">Technical Section Error</h2>
            <p class="text-gray-300">Failed to load technical details. Please check the console for errors.</p>
          </div>
        </div>
      </section>
    `;
...
// ====================================================================
// INITIALIZATION FUNCTIONS (tetap sama)
// ====================================================================

function initQaraaScripts() {
  console.log('✅ Qaraa scripts initialized');
  ...
  serviceCards.forEach(card => {
    ...
    card.addEventListener("mouseenter", () => {
      ...
      if (title) title.style.color = "#ad46ff";
    ...
    globalError.innerHTML = `
      ...
      ">
        ...
        ">
          <!-- GIF Plana -->
          <img 
            src="https://media1.tenor.com/m/Ps1f_GjbudoAAAAd/blue-archive-plana.gif" 
            alt="Plana Error"
            style="
              width: 200px;
              height: auto;
