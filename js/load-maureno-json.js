// load-maureno-json.js
document.addEventListener("DOMContentLoaded", async () => {
  try {
    console.log('🔄 Loading Maureno JSON data...');
    
    const response = await fetch('./js/maureno-data.json');
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    const data = await response.json();
    
    console.log('✅ Maureno JSON data loaded successfully');
    
    // Render semua sections untuk Maureno
    renderMaurenoNavbar(data.maureno_sections.navbar);
    renderMaurenoHome(data.maureno_sections.home);
    renderMaurenoAbout(data.maureno_sections.about);
    renderMaurenoTechnical(data.maureno_sections.technical);
    renderMaurenoCta(data.maureno_sections.cta);
    renderMaurenoFooter(data.maureno_sections.footer);
    renderMaurenoButtons(data.maureno_sections.buttons);
    
    initMaurenoScripts();
    
  } catch (error) {
    console.error('❌ Error loading Maureno JSON data:', error);
    showErrorOverlay('Failed to load Maureno sections data: ' + error.message);
  }
});

// ====================================================================
// RENDER FUNCTIONS UNTUK MAURENO
// ====================================================================

function renderMaurenoNavbar(navData) {
          ...
          navData.cta
            ? `<div class="hidden md:flex items-center">
                 <a href="${navData.cta.href}" 
                    class="bg-white text-black px-6 py-2 rounded-full hover:bg-amber-200 hover:text-white transition-colors">
                    ${navData.cta.text}
                 </a>
               </div>`
            : ''
...
function renderMaurenoHome(homeData) {
...
function renderMaurenoAbout(aboutData) {
...
function renderMaurenoTechnical(technicalData) {
...
function renderMaurenoCta(ctaData) {
...
function renderMaurenoFooter(footerData) {
...
function renderMaurenoButtons(buttonsData) {
...
// ====================================================================
// INITIALIZATION FUNCTIONS UNTUK MAURENO
// ====================================================================

function initMaurenoScripts() {
  console.log('✅ Maureno scripts initialized');
