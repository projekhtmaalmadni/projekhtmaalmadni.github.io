// load-pegasus-json.js
document.addEventListener("DOMContentLoaded", async () => {
  try {
    console.log('🔄 Loading Pegasus JSON data...');
    
    const response = await fetch('./js/pegasus-data.json');
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    const data = await response.json();
    
    console.log('✅ Pegasus JSON data loaded successfully');
    
    // Render sections
    renderPegasusNavbar(data.pegasus_sections.navbar);
    renderPegasusHome(data.pegasus_sections.home);
    renderPegasusAbout(data.pegasus_sections.about);
    renderPegasusTechnical(data.pegasus_sections.technical);
    renderPegasusCta(data.pegasus_sections.cta);
    renderPegasusFooter(data.pegasus_sections.footer);
    renderPegasusButtons(data.pegasus_sections.buttons);
    
    initPegasusScripts();
    
  } catch (error) {
    console.error('❌ Error loading Pegasus JSON data:', error);
    showErrorOverlay('Failed to load Pegasus sections data: ' + error.message);
  }
});

// Render functions for Pegasus
function renderPegasusNavbar(navData) {
          ...
          navData.cta
            ? `<div class="hidden md:flex items-center">
                 <a href="${navData.cta.href}" 
                    class="bg-white text-black px-6 py-2 rounded-full hover:bg-[#8ec5ff] hover:text-white transition-colors">
                    ${navData.cta.text}
                 </a>
               </div>`
            : ''
...
function renderPegasusHome(homeData) {
...
function renderPegasusAbout(aboutData) {
...
function renderPegasusTechnical(technicalData) {
...
function renderPegasusCta(ctaData) {
...
function renderPegasusFooter(footerData) {
...
function renderPegasusButtons(buttonsData) {
...
function initPegasusScripts() {
  console.log('✅ Pegasus scripts initialized');
  ...
  serviceCards.forEach(card => {
    ...
    card.addEventListener("mouseenter", () => {
      ...
      if (title) title.style.color = "#60a5fa";
