// load-belanja-json.js
document.addEventListener("DOMContentLoaded", async () => {
  try {
    console.log('🔄 Loading Belanja Smart JSON data...');
    
    const response = await fetch('./js/belanja-data.json');
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    const data = await response.json();
    
    console.log('✅ Belanja Smart JSON data loaded successfully');
    
    // Render semua sections untuk Belanja Smart
    renderBelanjaNavbar(data.belanja_sections.navbar);
    renderBelanjaHome(data.belanja_sections.home);
    renderBelanjaAbout(data.belanja_sections.about);
    renderBelanjaTechnical(data.belanja_sections.technical);
    renderBelanjaCta(data.belanja_sections.cta);
    renderBelanjaFooter(data.belanja_sections.footer);
    renderBelanjaButtons(data.belanja_sections.buttons);
    
    initBelanjaScripts();
    
  } catch (error) {
    console.error('❌ Error loading Belanja Smart JSON data:', error);
    showErrorOverlay('Failed to load Belanja Smart sections data: ' + error.message);
  }
});

// ====================================================================
// RENDER FUNCTIONS UNTUK BELANJA SMART
// ====================================================================

function renderBelanjaNavbar(navData) {
          ...
          navData.cta
            ? `<div class="hidden md:flex items-center">
                 <a href="${navData.cta.href}" 
                    class="bg-white text-black px-6 py-2 rounded-full hover:bg-[#7bf1a8] hover:text-white transition-colors">
                    ${navData.cta.text}
                 </a>
               </div>`
            : ''
...
function renderBelanjaHome(homeData) {
...
function renderBelanjaAbout(aboutData) {
...
function renderBelanjaTechnical(technicalData) {
...
function renderBelanjaCta(ctaData) {
...
function renderBelanjaFooter(footerData) {
          ...
          <div class="space-y-5">
            ${footerData.content.contact.items.map(item => `
              <div class="flex items-start space-x-3">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-green-400 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ${item.type === 'location' ? `
...
function renderBelanjaButtons(buttonsData) {
...
// ====================================================================
// INITIALIZATION FUNCTIONS UNTUK BELANJA SMART
// ====================================================================

function initBelanjaScripts() {
  console.log('✅ Belanja Smart scripts initialized');
