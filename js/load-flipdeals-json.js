// load-flipdeals-json.js
document.addEventListener("DOMContentLoaded", async () => {
  try {
    console.log('🔄 Loading Flip Deals JSON data...');
    
    const response = await fetch('./js/flipdeals-data.json');
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    const data = await response.json();
    
    console.log('✅ Flip Deals JSON data loaded successfully');
    
    // Render semua sections untuk Flip Deals
    renderFlipDealsNavbar(data.flipdeals_sections.navbar);
    renderFlipDealsHome(data.flipdeals_sections.home);
    renderFlipDealsAbout(data.flipdeals_sections.about);
    renderFlipDealsTechnical(data.flipdeals_sections.technical);
    renderFlipDealsCta(data.flipdeals_sections.cta);
    renderFlipDealsFooter(data.flipdeals_sections.footer);
    renderFlipDealsButtons(data.flipdeals_sections.buttons);
    
    initFlipDealsScripts();
    
  } catch (error) {
    console.error('❌ Error loading Flip Deals JSON data:', error);
    showErrorOverlay('Failed to load Flip Deals sections data: ' + error.message);
  }
});

// ====================================================================
// RENDER FUNCTIONS UNTUK FLIP DEALS
// ====================================================================

function renderFlipDealsNavbar(navData) {
          ...
          navData.cta
            ? `<div class="hidden md:flex items-center">
                 <a href="${navData.cta.href}" 
                    class="bg-white text-black px-6 py-2 rounded-full hover:bg-[#ffdf20] hover:text-white transition-colors">
                    ${navData.cta.text}
                 </a>
               </div>`
            : ''
...
function renderFlipDealsHome(homeData) {
...
function renderFlipDealsAbout(aboutData) {
...
function renderFlipDealsTechnical(technicalData) {
...
function renderFlipDealsCta(ctaData) {
...
function renderFlipDealsFooter(footerData) {
...
function renderFlipDealsButtons(buttonsData) {
...
// ====================================================================
// INITIALIZATION FUNCTIONS UNTUK FLIP DEALS
// ====================================================================

function initFlipDealsScripts() {
  console.log('✅ Flip Deals scripts initialized');
  ...
  serviceCards.forEach(card => {
    ...
    card.addEventListener("mouseenter", () => {
      ...
      if (title) title.style.color = "#eab308";
